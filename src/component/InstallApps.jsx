...
const InstallApps = ({b,data}) => {
  

    const handleUninstall = (id) => {
  
   
  const storedApp = getStoredBook();
  ...
  localStorage.setItem("insTall", JSON.stringify(remaining));
    ...
    return (
       ...
       <div>
                <button onClick={()=>handleUninstall(b.id)} className='border-0 rounded-sm px-5 py-2 text-white font-semibold bg-[#00D390]'>Uninstall</button>
